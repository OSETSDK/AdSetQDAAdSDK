//
//  AdSetQDANativeExpressBannerAd.h
//  AdSetQDAAdSDK
//
...
#import <AdSetQDAAdSDK/AdSetQDAAdSlot.h>

@class AdSetQDANativeExpressBannerAd;

NS_ASSUME_NONNULL_BEGIN

@protocol AdSetQDANativeExpressBannerAdDelegate <NSObject>
@optional

///  广告加载成功
///  在此方法中调用 showAdFromView:frame 方法
- (void)adsetQDA_nativeExpressBannerAdDidLoad:(AdSetQDANativeExpressBannerAd *)bannerAd;

/// 广告加载失败
- (void)adsetQDA_nativeExpressBannerAd:(AdSetQDANativeExpressBannerAd *)bannerAd didLoadFailWithError:(NSError *_Nullable)error;

/// 广告将要曝光
- (void)adsetQDA_nativeExpressBannerAdWillBecomVisible:(AdSetQDANativeExpressBannerAd *)bannerAd;

/// 广告曝光失败
- (void)adsetQDA_nativeExpressBannerAdBecomVisibleFail:(AdSetQDANativeExpressBannerAd *)bannerAd error:(NSError *_Nullable)error;

/// 广告被点击
- (void)adsetQDA_nativeExpressBannerAdDidClick:(AdSetQDANativeExpressBannerAd *)bannerAd;
 
/// 广告被关闭
- (void)adsetQDA_nativeExpressBannerAdClosed:(AdSetQDANativeExpressBannerAd *)bannerAd;

/// 广告详情页给关闭
- (void)adsetQDA_nativeExpressBannerAdViewDidCloseOtherController:(AdSetQDANativeExpressBannerAd *)bannerAd;

@end

@interface AdSetQDANativeExpressBannerAd : NSObject
...
/// 【必选参数】 广告的代理
@property(nonatomic,weak)id<AdSetQDANativeExpressBannerAdDelegate> delegate;
...
/// 初始化
/// AdSetQDAAdSlot 广告id
- (instancetype)initWithSlot:(AdSetQDAAdSlot *)slot size:(CGSize)size NS_DESIGNATED_INITIALIZER;
